...
def article_overview(kind, period):
    ...
    titles = []
    urls =[]
    # max_index = max(xrange(len(data)), key=lambda index: data[index][kind])
    # YOUR CODE HERE
    df = pd.DataFrame(data)

    # - list of dictionaries, where the dictionary key is "section" and value is "title"
    # - list of URLs for all media entries with "format": "Standard Thumbnail"
    # sections = list(set(df.section))
    # for section in sections:
    #     max_kind_of_section = df[df.section == section].sort("views",ascending=False).iloc[0]

    for datum in data:
        section = datum['section']
        titles.append({section:datum['title']})

        media_info_list = datum['media']
        for media_info in media_info_list:
            media_data_list = media_info["media-metadata"]
            for media_data_item in media_data_list:
                if media_data_item['format'] == "Standard Thumbnail":
                    urls.append(media_data_item['url'])
