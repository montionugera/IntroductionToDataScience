...
ggplot(redwine_data)+geom_histogram(aes(x =feature),binwidth = 0.1)+
  xlab('residual.sugar (g / cm^3)') + ylab('Number of observations')
summary(feature)
table(feature)
```
...
residual.sugar
The distribution for residual.sugar appears to be right skewed. Most of 
them (90%) residual.sugar less  than 3.6 (4.5 g / cm^3 are considered sweet).
...
ggplot(redwine_data)+geom_histogram(aes(x =feature),binwidth = 2)+
  xlab('free.sulfur.dioxide  (mg / dm^3)') + ylab('Number of observations')
summary(feature)
quantile(feature,.95)
...
Transform the long tail data to better understand the distribution of 
free.sulfur.dioxide
The free.sulfur.dioxide distribution is bimodal with the free.sulfur.dioxide 
peaking at 7 and 17.
...
ggplot(redwine_data)+geom_histogram(aes(x =feature),binwidth = 2)+
  xlab('total.sulfur.dioxide  (mg / dm^3)') + ylab('Number of observations')
summary(feature)
quantile(feature,.95)
...
ANS: The distribution for citric acid, volatile.acidity and free.sulfur.dioxide 
...
volatile.acid.V.wine_rating.plot<- build_bivarate_plot(
  ...
  scale_y_continuous(lim = c(0,1.6), breaks = seq(0,1.6,0.05))
...
alcohol.V.wine_rating.plot<-build_bivarate_plot(
  redwine_data, redwine_data$wine_rating, feature ,feature_name,'Wine Rating')+
  scale_y_continuous(lim = c(8,15), breaks = seq(8,15,0.1))
with(redwine_data, cor.test(feature, quality, method = 'pearson'))
...
(0.476)  
     For every pair of features free.sulfur.dioxide and total.sulfur.dioxide 
...
From the plot show that excellent wine mostly stay on the top left, good wine 
...
in alcohol grade "medium" on citric.acid at 0 and 0.4 is very high.


```{r echo=FALSE,warning=FALSE }
names(redwine_data)
wine_rating.v.alcohol.level.v.total.sulfur.dioxide.plot <- ggplot(
  aes(x = total.sulfur.dioxide ,fill = wine_rating), data = redwine_data) +
  #geom_point( alpha =  1/2 ) +
  xlab('total.sulfur.dioxide (mg/dm^3)') + ylab('Number of observations')+ 
  scale_fill_brewer(type = 'qual')+
  geom_histogram()+facet_wrap(~alcohol.level,ncol = 1, scales="free_y")+
  scale_x_continuous(lim = c(0,170), breaks = seq(0,170,5))
wine_rating.v.alcohol.level.v.total.sulfur.dioxide.plot
```
  
Wine rating.vs.alcohol.level.vs.total.sulfur.dioxide plot shows that excellent 
wine ratio in alcohol grade "medium" on total.sulfur.dioxide at 5-30 is very 
high.
...
It is very surprise that smell(total.sulfur.dioxide) has influnce over the wine 
...
### Description One
 ...
 The distribution for alcohol appears right skewed with the peaking at 9.5 % and
 slope down to 13.0 % with verry few of 8.0 % to 9.0 %, may be the demand of 
 wine are tend to be lower on higher percent alcohol and peak at 9.5 %.
...
cor.test(redwine_data$quality, redwine_data$alcohol)
cor.test(redwine_data$quality, redwine_data$volatile.acidity)
cor.test(redwine_data$quality, redwine_data$sulphates)
cor.test(redwine_data$quality, redwine_data$citric.acid)
```

### Description Two

Alcohol percentage, Sulphates, Volatile acidity correlate with wine rating 
positively but Volatile acidity correlate negatively.  
The alcohol variance in wine rating good and excellent are much larger than 
normal.  
The citric acid variance in wine rating good and normal are much larger than 
excellent.  
...
print_cor_value <- function(redwin_data_at.alcoholitem,alcohol.level.item,
                            i_feature_x_name, i_feature_x,i_feature_y) {
    title <- paste("On",alcohol.level.item,"level data -",
                    "correlation of wine quality","vs",i_feature_x_name,
                    "is :",cor.test(i_feature_x,i_feature_y)$estimate, sep=" ")
    #print( title)
    print( paste("On",alcohol.level.item," level data - wine rating","vs",
                 i_feature_x_name, sep=" "))
    print(by(i_feature_x, redwin_data_at.alcoholitem$wine_rating, summary))
}

alcohol.level.list <- c("low alcohol", "medium-low alcohol", "medium alcohol")
wine_rating.v.alcohol.level.v.volatile.acidity.plot

i_feature_x_name = 'volatile.acidity'
alcohol.level.item = 'any alcohol'
redwin_data_at.alcoholitem <- redwine_data
i_feature_x <- redwine_data$volatile.acidity
i_feature_y  <- redwine_data$quality

print_cor_value(redwin_data_at.alcoholitem,alcohol.level.item,
                i_feature_x_name,i_feature_x,i_feature_y)
for (alcohol.level.item in alcohol.level.list) {
    redwin_data_at.alcoholitem <- subset(redwine_data,
                 redwine_data$alcohol.level == alcohol.level.item)
    
    i_feature_x <- redwin_data_at.alcoholitem$volatile.acidity
    i_feature_y  <- redwin_data_at.alcoholitem$quality
...
i_feature_x_name = 'citric.acid'
alcohol.level.item = 'any alcohol'
redwin_data_at.alcoholitem <- redwine_data
i_feature_x <- redwine_data$citric.acid
i_feature_y  <- redwine_data$quality
print_cor_value(redwin_data_at.alcoholitem,alcohol.level.item,
                i_feature_x_name,i_feature_x,i_feature_y)
for (alcohol.level.item in alcohol.level.list) {
    ...
    i_feature_x <- redwin_data_at.alcoholitem$citric.acid
    i_feature_y  <- redwin_data_at.alcoholitem$quality
...
i_feature_x_name = 'total.sulfur.dioxide'
alcohol.level.item = 'any alcohol'
redwin_data_at.alcoholitem <- redwine_data
i_feature_x <- redwine_data$total.sulfur.dioxide
i_feature_y  <- redwine_data$quality
print_cor_value(redwin_data_at.alcoholitem,alcohol.level.item,
                i_feature_x_name,i_feature_x,i_feature_y)
for (alcohol.level.item in alcohol.level.list) {
    ...
    i_feature_x <- redwin_data_at.alcoholitem$total.sulfur.dioxide
    i_feature_y  <- redwin_data_at.alcoholitem$quality
...
i_feature_x_name = 'sulphates'
alcohol.level.item = 'any alcohol'
redwin_data_at.alcoholitem <- redwine_data
i_feature_x <- redwine_data$sulphates
i_feature_y  <- redwine_data$quality
print_cor_value(redwin_data_at.alcoholitem,alcohol.level.item,
                i_feature_x_name,i_feature_x,i_feature_y)
for (alcohol.level.item in alcohol.level.list) {
    ...
    i_feature_x <- redwin_data_at.alcoholitem$sulphates
    i_feature_y  <- redwin_data_at.alcoholitem$quality
...
On low alcohol percentage exellent wine quality rarely to be found mostly is 
normal rating wine. 
On medium-low alcohol percentage 75% of exellent wine can be found in
total.sulfur.dioxide below 52 mg / dm^3 or volatile.acidity below 0.6 g/cm^3 
but mostly are normal and good rating wine.  
On mediumw alcohol percentage 75% of exellent wine can be found on :
total.sulfur.dioxide below 45.75 mg / dm^3 or sulphate upper than 0.64 g/dm^3 
and mostly are exellent and good rating wine.  
...
The data set contain 1599 wine from 2009. I start by understand the variables in
data set and try to interpret in term of sense that human can percieve. 
Surprisingly, I found that the taste sourness and salty has no evidence that 
they has influence over the quality of wine but the smell 
...
wine,On mediumw alcohol percentage wine exellent can be found at high percentage
on total sulfur dioxide below 50 and sulphate upper than 0.65 and mostly are 
exellent and good rating wine. I struggled to visulize multivariate plot 
to clearly present the relation more than one featue against wine quality at 
first finally I found out that if I create new varible that represent the 
feature as group It will be easier,Next I can not clearly present the 
relation of selected features by geom_point as you can see the correlation value
 not so high for each feature but this become much better when I decide to 
use histogram.

Finally I am very enjoyed to make this analysis,It makes me more understand how
to set the questions and solve them.
